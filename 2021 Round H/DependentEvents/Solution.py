...
# Need to uncomment next line to run for larger cases
...
class Helper:
  ...
  def dfs(self, graph, node, h=0, parent=-1):
    ...
    if parent == -1:
      ...
      self.uncond_prob[node] = mod_mul(self.uncond_prob[parent],self.probs[node][0]) + mod_mul(MOD+1-self.uncond_prob[parent], self.probs[node][1])
      self.uncond_prob[node] %= MOD
    ...
    while 2**i <= h:
      ...
      self.cond_prob_h0[node][i] = self.cond_prob_h0[node][i-1]*self.cond_prob_h0[par][i-1] + self.cond_prob_h1[node][i-1]*(MOD+1-self.cond_prob_h0[par][i-1])
      self.cond_prob_h1[node][i] = self.cond_prob_h0[node][i-1]*self.cond_prob_h1[par][i-1] + self.cond_prob_h1[node][i-1]*(MOD+1-self.cond_prob_h1[par][i-1])
      self.cond_prob_h0[node][i] %= MOD
      self.cond_prob_h1[node][i] %= MOD
      i += 1
  ...
  def get_cond_prob_for_k_ancestors(self, node, k, cond_prob, neg=False):
    ...
    mid_cp = self.get_cond_prob_for_k_ancestors(mid, k-2**i, cond_prob, neg)
    
    # merge
    res = mid_cp * self.cond_prob_h0[node][i] + (MOD+1-mid_cp) * self.cond_prob_h1[node][i]
    res %= MOD
...
def solve(): 
  ...
  scale_inv = mod_inv(SCALE)
  # print("test")

  graph = defaultdict(list)
  probs = []
  first = int(input())
  probs.append((mod_mul(first, scale_inv), mod_mul(first, scale_inv)))
  for i in range(1, N):
    parent, p1, p2 = list(map(int, input().split()))
    parent -= 1 # we number from 0 to N-1
    graph[parent].append(i)
    probs.append((mod_mul(p1, scale_inv), mod_mul(p2, scale_inv)))
